...
Or use the agent to run the full pipeline automatically:

```
e2e-runner agent → auto-detects Create mode → analyze → plan → create → run → report
```
...
All `/e2e:*` commands and the e2e-runner agent set `E2E_REPORT_NAME` automatically. When running tests manually, set it yourself:
...
| Type | Path | Notes |
|------|------|-------|
| HTML | `playwright/reports/{page-name}/` | Auto-generated by Playwright; requires `E2E_REPORT_NAME` |
| Markdown | `playwright/reports/{page-name}/test-report.md` | Generated by agent/command in Traditional Chinese |
| Analysis | `playwright/reports/{page-name}/analysis.md` | Page structure analysis from `/e2e:analyze` |
| Coverage Plan | `playwright/reports/{page-name}/coverage-plan.md` | Test coverage plan from `/e2e:plan` |
